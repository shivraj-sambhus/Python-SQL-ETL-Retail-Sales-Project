...
#This API was sourced from OpenWeathermap. I have deactivated and deleted this key for safety purposes.
...
#The zip code 90004 corresponds to the first store, LA001.
print(pd.read_sql("SELECT * FROM Sales WHERE store_zip=90004 AND temp > 65", con=conn))

#LOAD PHASE END

# I want to create a scatterplot visualization of the temperature versus revenue. I don't want to do this for each store, so I'll pick the first one, LA001.
...
plt.title('Revenue versus Humidity with heat for Temperature')
plt.xlabel('Revenue ($)')
plt.ylabel('Humidity (%)')

plt.legend(title='Temperature (F)', bbox_to_anchor=(1.05, 1), loc='upper left')
